defmodule Tilde.Tool.Event do
  @moduledoc "Structured payload for tool lifecycle updates."

  @type status :: :preparing | :running | :success | :error | :cancelled | atom() | String.t()
  ...
  def preparing(fields) do
    fields
    |> Keyword.put_new(:status, :preparing)
    |> Keyword.put_new(:phase, :preparing)
    |> build()
  end

  @spec started(keyword()) :: t()
  def started(fields) do
    fields
    |> Keyword.put_new(:status, :running)
    |> Keyword.put_new(:phase, :started)
    |> build()
  end
  ...
  def finished(fields) do
    ...
    |> Keyword.put_new(:phase, :finished)
    |> build()
  end

  @spec normalize_result(term()) :: term()
  def normalize_result({:ok, result, _meta}), do: result
  def normalize_result({:ok, result}), do: result
  def normalize_result({:error, reason, _meta}), do: %{error: reason}
  ...
  defp status_from_output(_result), do: :success
